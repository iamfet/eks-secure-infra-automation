...
resource "aws_vpc_security_group_ingress_rule" "istio-gateway-lb_ipv4" {
...
resource "aws_vpc_security_group_ingress_rule" "istio-gateway-lb_ipv6" {
  security_group_id = aws_security_group.istio-gateway-lb.id
  cidr_ipv6         = module.vpc.ipv6_cidr_block
  from_port         = 443
  ip_protocol       = "tcp"
  to_port           = 443
}
